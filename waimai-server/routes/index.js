  ...
  UserModel.findOne({name}, function (err, user) {
    if (user) {
      ...
      if (user.pwd !== pwd) {
        ...
        res.send({code: 0, data: {_id: user._id, name: user.name, phone: user.phone, sex: user.sex, birthday: user.birthday, area: user.area, occuption: user.occuption}})
      }
    } else {
      const userModel = new UserModel({name, pwd, sex, birthday, area, occuption})
      userModel.save(function (err, user) {
        // 向浏览器端返回cookie(key=value)
        // res.cookie('userid', user._id, {maxAge: 1000*60*60*24*7})
        req.session.userid = user._id
        const data = { _id: user._id, name: user.name, phone: user.phone, sex: user.sex, birthday: user.birthday, area: user.area, occuption: user.occuption}
        // 3.2. 返回数据(新的user)
...
router.post('/login_sms', function (req, res, next) {
  ...
  UserModel.findOne({phone}, function (err, user) {
    if (user) {
      ...
      //存储数据
      const userModel = new UserModel({phone, sex, birthday, area, occuption})
      userModel.save(function (err, user) {
        req.session.userid = user._id
        res.send({code: 0, data: user})
      })
...
更新用户信息
 */
router.get('/updateInf/:propery', function(req, res) {
  const userid = req.session.userid
  const {propery} = req.params
  const newValue = req.query[propery]
  const data = {}
  data[propery] = newValue
  UserModel.updateOne({_id: userid}, data, function(err) {
    if(err) throw err
